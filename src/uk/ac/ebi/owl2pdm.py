...
from org.semanticweb.owlapi.util import SimpleShortFormProvider, SimpleIRIShortFormProvider
...
class ont_manager():
    ...
    def __init__(self, ont = '', file_path='', uri=''):
        ...
        if not ont:
            if file_path:
                ...
                self.ont = self.manager.loadOntologyFromIRI(IRI.create(uri))      # check this!          
            else:
    ...
    def get_all_class_sfids(self):
        cs = self.ont.getClassesInSignature()
        return [self.bi_sfp.getShortForm(e) for e in cs]           
    ...
    def get_subsets(self, owl_entity_sfid, return_sf = True):
        """Get subsets annotations on term
        Default is to return list of short_forms, 
        but if return_sf = "False" it returns a list of IRI objects"""
        e = self.bi_sfp.getEntity(owl_entity_sfid)
        ap = self.bi_sfp.getEntity('inSubset')
        annotations = e.getAnnotations(self.ont, ap)
        if return_sf:
            return [self.simple_iri_sfp.getShortForm(i.getValue()) for i in annotations]
        else:   
            return [i.getValue() for i in annotations]
